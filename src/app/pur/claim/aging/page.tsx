  ...
  return (
    <div>
      ...
      <div className="bg-[#f59e0b]/10 border border-[#f59e0b]/40 px-4 py-3 mb-4 flex items-center gap-3">
        ...
        <span className="text-sm font-label text-[#f59e0b]">AGING 이슈 {MOCK.length}건 — 장기 미처리 항목 포함</span>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="font-headline text-sm">
            {MOCK.sort((a,b)=>b.overdueDays-a.overdueDays).map(r=>(
              <tr key={r.po} className={`border-b border-outline-variant/5 ${r.overdueDays>=14?"bg-error/5":r.overdueDays>=7?"bg-[#f59e0b]/5":""}`}>
                ...
                <td className={`px-4 py-2 tabular-nums font-black ${r.overdueDays>=14?"text-error":r.overdueDays>=7?"text-[#f59e0b]":"opacity-70"}`}>{r.overdueDays}일</td>
                <td className="px-4 py-2 tabular-nums text-xs">{r.poQty}</td>
                <td className="px-4 py-2 tabular-nums text-xs">{r.recvQty}</td>
                <td className="px-4 py-2 text-xs font-bold text-[#f59e0b]">{r.diff}</td>
                <td className="px-4 py-2 w-32">
                  <div className="bg-surface-container h-2">
                    <div className="h-2 bg-[#f59e0b]" style={{width:`${Math.min(100,(r.overdueDays/maxDays)*100)}%`}} />
                  </div>
                </td>
              </tr>
            ))}
