      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {CHANNELS.map(c=>(
          <button key={c} onClick={()=>setCh(c)}
            className={`px-3 py-1 text-xs font-label uppercase tracking-widest border ${ch===c?"bg-primary-accent text-white border-primary-accent":"bg-surface-container border-outline-variant/20"}`}>
            {c}
          </button>
        ))}
      </div>
