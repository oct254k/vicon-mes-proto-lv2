  ...
  return (
    <div>
      ...
      <div className="bg-warning/10 border border-warning/40 px-4 py-3 mb-4 flex items-center gap-3">
        ...
        <span className="text-sm font-label text-warning">지연 ASN {MOCK.length}건 — 구매팀 조치 필요</span>
      </div>
      ...
      <div className="bg-surface-container-lowest overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="font-headline text-sm">
            {MOCK.map(r=>(
              <tr key={r.id} className="border-b border-outline-variant bg-warning/5 hover:bg-warning/10">
                ...
                <td className="px-4 py-2 tabular-nums text-xs text-warning font-bold">{r.etaNew}</td>
                <td className="px-4 py-2 tabular-nums text-xs text-warning font-black">+{r.delayDays}일</td>
