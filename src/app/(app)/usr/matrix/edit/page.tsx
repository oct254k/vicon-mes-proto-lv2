  ...
  return (
    <div>
      ...
      <div className="bg-surface-container-low border-l-4 border-warning p-4 mb-4 flex items-center gap-3">
        <span className="text-warning font-bold text-sm font-label uppercase tracking-widest">주의</span>
      ...
      <div className="overflow-x-auto">
        <table className="text-left border-collapse text-xs font-headline whitespace-nowrap">
          ...
          <tbody>
            {LEVELS.map((lv) =>
              DEPTS.map((dept, di) => (
                <tr key={`${lv}-${dept}`} className={`border-b border-outline-variant hover:bg-surface-container-highest/20 ${di === 0 ? "border-t border-outline-variant/20" : ""}`}>
      ...
      <div className="flex gap-3 mt-4">
        <a href="/usr/matrix/impact" className="px-5 py-2 bg-warning text-black text-xs font-label uppercase tracking-widest font-bold">영향 평가 실행</a>
