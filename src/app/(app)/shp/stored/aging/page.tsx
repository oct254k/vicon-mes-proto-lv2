  ...
  return (
    <div>
      ...
      <div className="bg-warning/10 border border-warning/40 px-4 py-3 mb-4 flex items-center gap-3">
        ...
        <span className="text-sm font-label text-warning">장기 체류 PKG {MOCK.length}건 — 즉시 출하 검토 필요</span>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="font-headline text-sm">
            {MOCK.sort((a,b)=>b.storedDays-a.storedDays).map(r=>(
              <tr key={r.id} className={`border-b border-outline-variant ${r.risk==="HIGH"?"bg-error/5":r.risk==="MEDIUM"?"bg-warning/5":""}`}>
                ...
                <td className={`px-4 py-2 font-black tabular-nums ${r.risk==="HIGH"?"text-error":r.risk==="MEDIUM"?"text-warning":"opacity-70"}`}>{r.storedDays}일</td>
                ...
                <td className="px-4 py-2 w-28">
                  <div className="bg-surface-container h-2">
                    <div className={`h-2 ${r.risk==="HIGH"?"bg-error":r.risk==="MEDIUM"?"bg-warning":"bg-primary-accent"}`} style={{width:`${(r.storedDays/max)*100}%`}} />
                  </div>
                </td>
