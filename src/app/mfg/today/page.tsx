...
const SESSION = { name: "홍길동", plant: "P3000 제3 이천공장 (데크)", wc: "신선공정", shift: "주간조" };
...
const STATUS_MAP: Record<string, { badge: "running" | "idle" | "warning" | "stopped"; label: string; dot: string }> = {
  IN_PROGRESS: { badge: "running", label: "IN PROGRESS", dot: "bg-yellow-400" },
  PENDING:     { badge: "idle",    label: "PENDING",     dot: "bg-gray-400" },
  COMPLETED:   { badge: "running", label: "COMPLETED",   dot: "bg-[#00912F]" },
};
          ...
          return (
            <div key={line.id} className="bg-surface-container-low border border-outline-variant/10 p-4 space-y-2">
              ...
              <div className="flex gap-2 pt-1">
                {line.status === "PENDING" && (
                  <a href="/mfg/issue" className="flex-1 bg-primary-accent text-white text-center py-4 text-base font-label uppercase tracking-widest">
                    자재 투입
                  </a>
                )}
                {line.status === "IN_PROGRESS" && (
                  <>
                    <a href="/mfg/issue" className="flex-1 bg-surface-container border border-primary-accent/50 text-primary-accent text-center py-4 text-base font-label uppercase tracking-widest">
                      ISSUE
                    </a>
