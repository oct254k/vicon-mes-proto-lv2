...
const STATUS_MAP: Record<string, { type: "running" | "stopped" | "warning" | "idle"; label: string }> = {
  SCHEDULED:   { type: "idle",    label: "SCHEDULED" },
  IN_PROGRESS: { type: "warning", label: "IN_PROGRESS" },
  COMPLETED:   { type: "running", label: "COMPLETED" },
...
export default function SHPSchedulePage() {
  ...
  const vehicleRows = VEHICLES.map((v) => ({
    ...
    status: <StatusBadge type={STATUS_MAP[v.status]?.type ?? "idle"} label={v.status} /> as unknown as string,
  }));
            ...
            <div className="p-2 space-y-2 min-h-[120px]">
              {SHIPMENTS.filter((s) => s.date === `2026-05-0${day.split("/")[1]}`).map((s) => (
                <button key={s.id} onClick={() => setSelected(s)}
                  ...
                  <StatusBadge type={STATUS_MAP[s.status]?.type ?? "idle"} label={s.status} />
                </button>
              ))}
            </div>
          </div>
