    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2 flex-wrap text-xs font-label">
          {REASONS.map(r=><span key={r} className="px-2 py-1 bg-surface-container border border-[#f59e0b]/30 text-[#f59e0b]">{r}</span>)}
        </div>
          ...
          <tbody className="font-headline text-sm">
            {MOCK.map(r=>(
              <tr key={r.id} className="border-b border-outline-variant/5 hover:bg-surface-container-highest/20">
                ...
                <td className="px-4 py-2"><span className="text-[#f59e0b] text-xs font-label font-bold">{r.reason}</span></td>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-surface-container-lowest border border-outline-variant/20 p-6 w-96 max-w-full">
            <h3 className="font-headline font-black text-sm uppercase tracking-widest mb-4 text-[#f59e0b]">불일치 보고</h3>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={()=>setShowForm(false)} className="px-4 py-2 bg-[#f59e0b] text-black text-xs font-label uppercase">보고 등록</button>
            </div>
          </div>
        </div>
      )}
